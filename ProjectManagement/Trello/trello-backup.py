...
# Added buy PM to later incorporate date into filename
import datetime
FILE_DATE = datetime.datetime.now().isoformat()[:16].replace(":","")+"---"
...
def boardFilename(output_dir, board, epoch_time):
    ...
    # only bit I have added is to append the date to the front of the fielnane
    formatted = FILE_DATE + formatted
